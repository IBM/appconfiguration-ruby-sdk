...
class FileManager
  ...
  def read_bootstrap_configurations_from_file(file_path)
    unless File.exist?(file_path)
      raise "given bootstrap file path doesn't exist: #{file_path}"
    end

    data = File.read(file_path).strip

    if data.empty?
      raise "given bootstrap file is empty: #{file_path}"
    end

    begin
      data
    rescue StandardError => err
      raise "failed to parse the json from the given bootstrap file: #{file_path}. Error #{err}"
    end
  end
