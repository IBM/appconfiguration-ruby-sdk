...
def validate_resource(resource, collection)
  ...
  raise IbmAppconfigurationRubySdk::Error, "Improper collection format in resource data" unless collections.is_a?(Array)
...
def extract_environment_data(data, environment_id)
  unless data.key?(:segments) && data[:segments].is_a?(Array) &&
    ...
    raise IbmAppconfigurationRubySdk::Error, "Improper Data format present in configuration"
  end
  ...
  raise IbmAppconfigurationRubySdk::Error, "Matching environment not found in configuration"
end
...
def extract_resources(resource_data, collection)
  ...
  raise IbmAppconfigurationRubySdk::Error, "Required segment doesn't exist in provided segments" if segment_ids.size.positive?
...
def extract_configurations(configurations, environment, collection)
  ...
  raise IbmAppconfigurationRubySdk::Error, "Improper/Missing collections in configuration" unless configurations.key?(:collections) && configurations[:collections].is_a?(Array)

  match_found = configurations[:collections].any? { |coll| coll[:collection_id] == collection }

  raise IbmAppconfigurationRubySdk::Error, "Required collection not found in collections" unless match_found
...
end # IbmAppconfigurationRubySdk
