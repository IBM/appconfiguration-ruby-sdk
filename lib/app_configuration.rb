...
class AppConfiguration
  ...
  def set_context(collection_id, environment_id, options = {})
    ...
    unless @is_initialized
      report_error(Constants::COLLECTION_ID_ERROR)
    end

    unless collection_id
      report_error(Constants::COLLECTION_ID_VALUE_ERROR)
    end

    unless environment_id
      report_error(Constants::ENVIRONMENT_ID_VALUE_ERROR)
    end
    ...
    if options
      unless options.is_a?(Hash)
        report_error("#{Constants::INVALID_OPTIONS_PARAMETER}")
      end
      ...
      if default_options[:live_config_update_enabled] == false && default_options[:bootstrap_file].nil?
        report_error(Constants::CONFIGURATION_FILE_NOT_FOUND_ERROR)
      end
    end
  ...
  def get_feature(feature_id)
    if @is_initialized && @is_initialized_config
      return @configuration_handler.get_feature(feature_id)
    end
  ...
  def get_features
    if @is_initialized && @is_initialized_config
      return @configuration_handler.get_features
    end
    @logger.error(Constants::COLLECTION_INIT_ERROR)
    nil
  end
  ...
  def get_property(property_id)
    if @is_initialized && @is_initialized_config
      return @configuration_handler.get_property(property_id)
    end
    @logger.error(Constants::COLLECTION_INIT_ERROR)
    nil
  end
  ...
  def get_properties
    if @is_initialized && @is_initialized_config
      return @configuration_handler.get_properties
    end
    @logger.error(Constants::COLLECTION_INIT_ERROR)
    nil
  end
  ...
  def get_secret(property_id, secrets_manager_service)
    if @is_initialized && @is_initialized_config
      if secrets_manager_service
        return @configuration_handler.get_secret(property_id, secrets_manager_service)
      end
  ...
  def track(event_key, entity_id)
    if @is_initialized && @is_initialized_config
      return @configuration_handler.track(event_key, entity_id)
    end
    @logger.error(Constants::COLLECTION_INIT_ERROR)
    nil
  end
  ...
  def connected?
    if @is_initialized && @is_initialized_config
      return @configuration_handler.connected?
    end
    false
  end
