...
RSpec.describe ConfigurationHandler do
        ...
        { name: "Segment Progressive", feature_id: "f-seg-progressive", type: "BOOLEAN",
          ...
          segment_rules: [
            { rules: [{ segments: ["seg-ibm"] }], value: "$default", order: 1,
              rule_id: "rule-1", rollout_type: Constants::PROGRESSIVE,
  ...
  describe "lookups" do
    it "returns loaded features, properties and segments" do
      expect(handler.get_feature("f-enabled")).to be_a(Feature)
      expect(handler.get_property("show-ad")).to be_a(Property)
      expect(handler.get_segment("seg-ibm")).to be_a(Segment)
    end
